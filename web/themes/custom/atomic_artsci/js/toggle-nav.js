class ToggleNavBehavior {
  constructor(context) {
    ...
    this.artsciHeader = this.context.querySelector('[data-artsci-header]');

    // Only do something if we have a match for our element.
    if (this.artsciHeader) {
      ...
      // Set positioning of canvasDrawer based on artsciBar height
      this.artsciBarHeight = this.artsciHeader.offsetHeight;
      this.canvasDrawer.style.top = `${this.artsciBarHeight}px`;
  ...
  updateCanvasDrawerPosition() {
    const headerPosition = this.artsciHeader.getBoundingClientRect().top;
    this.canvasDrawer.style.top = `${Math.max(this.artsciBarHeight + headerPosition, 0)}px`;
  }

  // Function to handle resizing and updating artsciBarHeight and canvasDrawer position
  handleResize() {
    this.artsciBarHeight = this.artsciHeader.offsetHeight;
...
// Setup event listeners
  setupEventListeners() {
    ...
    observer.observe(this.artsciHeader, { attributes: true, subtree: true });
  }
