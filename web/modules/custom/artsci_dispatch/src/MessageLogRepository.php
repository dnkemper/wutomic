...
namespace Drupal\artsci_dispatch;
 ...
 * 'artsci_dispatch.message_log_repository'.
 *
 * This repository includes basic CRUD behaviors.
 *
 * @ingroup artsci_dispatch_messages_log
 */
class MessageLogRepository {
  ...
  public function insert(array $entry) {
    try {
      $return_value = $this->connection->insert('artsci_dispatch_messages_log')
        ->fields($entry)
        ->execute();
    }
    catch (\Exception $e) {
  ...
  public function update(array $entry) {
    try {
      ...
      $count = $this->connection->update('artsci_dispatch_messages_log')
        ->fields($entry)
  ...
  public function delete(array $entry) {
    $this->connection->delete('artsci_dispatch_messages_log')
  ...
  public function load(array $entry = []) {
    // Read all the fields from the artsci_dispatch_messages_log table.
    $select = $this->connection
      ->select('artsci_dispatch_messages_log')
      // Add all the fields into our select query.
      ->fields('artsci_dispatch_messages_log');
