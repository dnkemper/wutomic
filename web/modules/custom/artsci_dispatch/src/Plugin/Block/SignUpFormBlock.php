...
namespace Drupal\artsci_dispatch\Plugin\Block;
 ...
 *
 * @Block(
 *   id = "artsci_dispatch_sign_up_form",
 *   admin_label = @Translation("Sign up form"),
 *   category = @Translation("Artsci Dispatch")
 * )
 */
class SignUpFormBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function blockForm($form, FormStateInterface $form_state) {
    ...
    if ($populations === FALSE) {
      $form['invalid_api_key'] = [
        '#prefix' => '<div>',
        '#suffix' => '</div>',
        '#markup' => $this->t('Either there is no API key for Artsci Dispatch currently configured, or the one you currently have is not valid. Please double check the API key set in the <a href=":url">Dispatch configuration</a>.', [
          ':url' => Url::fromRoute('artsci_dispatch.settings_form')->toString(),
        ]),
      ];
  ...
  public function build() {
    ...
    if (!empty($this->configFactory->get('artsci_dispatch.settings')->get('api_key'))) {
      ...
      $build['form'] = $this->formBuilder->getForm('Drupal\artsci_dispatch\Form\SubscribeForm', $this->configuration['population']);
