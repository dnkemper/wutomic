...
namespace Drupal\artsci_dispatch\Form;
...
use Drupal\artsci_dispatch\DispatchApiClientInterface;
...
class ThankYouForm extends FormBase {
   ...
   *   The JSON controller.
   * @param \Drupal\artsci_dispatch\DispatchApiClientInterface $dispatch
  ...
  public function __construct(protected ClientInterface $client, protected Json $json, protected DispatchApiClientInterface $dispatch) {
    $this->dispatch->setKey($this->config('artsci_dispatch.settings')->get('thanks.api_key'));
  }
  ...
  public static function create(ContainerInterface $container) {
    return new static(
      ...
      $container->get('artsci_dispatch.dispatch_client'),
    );
  }
  ...
  public function getFormId() {
    return 'artsci_dispatch_thankyou_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['to_email'] = [
      ...
      '#description' => $this->t('An <em>@artsci.edu</em> email address. Look up an email address in our <a target="_blank" href="https://iam.artsci.edu/whitepages/search">directory</a>.'),
      '#required' => TRUE,
      '#pattern' => '.+@artsci\.edu',
    ];
    ...
    if ($this->config('artsci_dispatch.settings')->get('thanks.approval')) {
      $form['approval'] = [
        '#type' => 'checkbox',
    ...
    if ($this->config('artsci_dispatch.settings')->get('thanks.supervisor')) {
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    $endpoint = ($env === 'prod' || $env === 'test') ? 'https://data.its.artsci.edu/hris/supervisors' : 'https://data-test.its.artsci.edu/hris/supervisors';
    $config = $this->config('artsci_dispatch.settings');
    ...
    try {
      ...
      $this->logger('artsci_dispatch')->error($this->t('HR API error: @error.', [
        '@error' => $e->getMessage(),
      ]));
      ...
      if ((int) $e->getCode() === 404) {
        ...
        $form_state->setError($form, $this->t('An error was encountered processing the form. If the problem persists, please contact the <a href=":link">ITS Help Desk</a>.', [
          ':link' => 'https://its.artsci.edu/contact',
        ]));
      }
    }
  }
