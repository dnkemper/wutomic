...
namespace Drupal\washu_calendar_subscription\Controller;
...
class AcademicCalendarIcalController extends ControllerBase {
  ...
  public function feed(Request $request): Response {
    ...
    // This is a public feed of published calendar events, fetched anonymously
    // by calendar apps, so it must not apply the requesting user's node access
    // grants (which would return nothing for anonymous on sites with node
    // access control). Publication status is still enforced below.
    $query = $node_storage->getQuery()
      ->condition('type', 'academic_calendar_event')
      ->condition('status', 1)
  ...
  protected function buildIcal(array $nodes, Request $request): string {
    ...
    $lines[] = 'VERSION:2.0';
    $lines[] = 'PRODID:-//Washington University in St. Louis//Academic Calendar//EN';
    $lines[] = 'CALSCALE:GREGORIAN';
    $lines[] = 'METHOD:PUBLISH';
    $lines[] = 'X-WR-CALNAME:Academic Calendar';
    ...
    foreach ($nodes as $node) {
      ...
      if ($node->hasField('field_external_url') && !$node->get('field_external_url')->isEmpty()) {
        $url = $node->get('field_external_url')->value;
      }
