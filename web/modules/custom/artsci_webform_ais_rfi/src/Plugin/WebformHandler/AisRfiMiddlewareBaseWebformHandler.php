...
namespace Drupal\artsci_webform_ais_rfi\Plugin\WebformHandler;
...
abstract class AisRfiMiddlewareBaseWebformHandler extends WebformHandlerBase {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    // Load configuration.
    $config = $this->configFactory->get('artsci_webform_ais_rfi.settings');
    ...
    if (!isset($auth['user']) || !isset($auth['pass'])) {
      $form['missing_auth'] = [
        '#markup' => $this->t('<strong>Warning:</strong> The AIS RFI Middleware authentication credentials are missing. Please contact the Artsci team for assistance.'),
        '#weight' => -100,
      ];
    }
  ...
  public function postSave(WebformSubmissionInterface $webform_submission, $update = TRUE) {
    ...
    if (!$endpoint_url) {
      ...
      $endpoint_url = ($env === 'prod') ? 'https://apps.its.artsci.edu/prospect-api/api/prospect/submit' : 'https://test.its.artsci.edu/prospect-api/api/prospect/submit';
    }
