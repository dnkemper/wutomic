...
namespace Drupal\artsci_events\Controller;
...
class EventsController extends ControllerBase {
  ...
  public function build($event_id, $event_instance) {
    ...
    if ($this->config('artsci_events.settings')->get('event_link') === 'event-link-external') {
  ...
  protected function getEventData($event_id) {
    return artsci_events_load([], ['node', "{$event_id}.json"]);
  }
