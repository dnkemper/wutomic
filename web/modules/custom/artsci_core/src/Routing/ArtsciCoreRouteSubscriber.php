...
namespace Drupal\artsci_core\Routing;
 ...
 */
class ArtsciCoreRouteSubscriber extends RouteSubscriberBase {
  ...
  protected function alterRoutes(RouteCollection $collection) {
    ...
    if ($route = $collection->get('system.site_information_settings')) {
      ...
      $route->setDefault('_form', 'Drupal\artsci_core\Form\ArtsciCoreSiteInformationForm');
    }
    ...
    foreach ($restricted_routes as $restricted_route) {
      if ($route = $collection->get($restricted_route)) {
        $route->setRequirement('_artsci_core_access_check', 'TRUE');
      }
    }
  }
