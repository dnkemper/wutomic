...
class P2LbConverter {
   ...
   *
   * @var \Drupal\artsci_pages\Entity\Page
   */
  protected Page $page;
  ...
  public function convert(): void {
    ...
    artsci_p2lb_clear_tempstore($this->page);
  }
  ...
  protected function processSections() {
    ...
    $section_ids = artsci_p2lb_fetch_child_ids($this->page);
  ...
  protected function processSection($revision_id) {
    ...
    $section_styles = artsci_p2lb_section_styles($paragraph_section);

    // Get all paragraphs attached to this section.
    $pvids = artsci_p2lb_fetch_child_ids($paragraph_section);
    ...
    if ($section_image_fid) {
      ...
      $section_array = artsci_p2lb_section_image($section_image_fid, $banner_text, $this->page);
    ...
    $sections_infos = artsci_p2lb_determine_columns($col_widths);
    ...
    foreach ($sections_infos as $delta => $section_info) {
      ...
      $col_width_str = artsci_p2lb_multicol_settings($section_info['col_widths']);
      $layout_settings = [
        'label' => $section_title,
      ...
      $section_array = artsci_p2lb_create_section_array($layout_id, $layout_settings);
      ...
      for ($i = 0; $i < $num_cols; $i++) {
        ...
        $block_configs = artsci_p2lb_process_paragraph($paragraphs[$key], $this->page);
  ...
  protected function processMenu() {
    if (!in_array('no_sidebars', array_column($this->page->get('field_publish_options')
      ...
      if (!empty($menu_children)) {
        ...
        $section_array = artsci_p2lb_create_section_array('layout_page', $layout_settings, $components);
  ...
  protected function addLockedSections() {
    ...
    $default_sections = _artsci_p2lb_get_page_lb_sections_config();
