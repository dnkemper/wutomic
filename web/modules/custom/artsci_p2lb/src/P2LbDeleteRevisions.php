  ...
  public static function deleteRevisions(int $batch_id, array $nids, array &$context) {
    ...
    foreach ($nids as $nid) {
      ...
      if ($protected_vid) {
        ...
        \Drupal::logger('artsci_p2lb')->notice('Deleted previous revisions for Node ID: ' . $nid);
      }
      else {
        \Drupal::logger('artsci_p2lb')->notice('Whoa there! There is no protected revision for Node ID: ' . $nid);
      }
    }
