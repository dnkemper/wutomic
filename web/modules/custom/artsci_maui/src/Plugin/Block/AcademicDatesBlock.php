...
namespace Drupal\artsci_maui\Plugin\Block;
...
use Drupal\artsci_maui\MauiApi;
...
class AcademicDatesBlock extends BlockBase implements ContainerFactoryPluginInterface {
   ...
   *
   * @var \Drupal\artsci_maui\MauiApi
   */
  protected $maui;
   ...
   * @param \Drupal\artsci_maui\MauiApi $maui
   *   The artsci_maui.api service.
  ...
  public function blockForm($form, FormStateInterface $form_state) {
    ...
    $form['display_more_link'] = [
      ...
      '#description' => $this->t('Check to include a "display more" link. Default is https://registrar.artsci.edu/academic-calendar. Alternatively, a custom URL path can be provided in the ‘Path’ text box below.'),
    ...
    $form['more_link'] = [
      ...
      '#default_value' => isset($config['more_link']) ? LinkHelper::getUriAsDisplayableString($config['more_link']) : 'https://registrar.artsci.edu/academic-calendar',
      '#element_validate' => [
        [
          LinkWidget::class,
          'validateUriElement',
  ...
  public function build() {
    ...
    $build = [
      '#attached' => [
        'library' => 'artsci_maui/session_dates',
      ],
    ];
    ...
    $build['form'] = $this->formBuilder->getForm(
      '\Drupal\artsci_maui\Form\AcademicDatesForm',
      $config['session'],
      $config['category'],
      $child_heading_size,
    ...
    if ($config['display_more_link'] === TRUE) {
      $more_link = $config['more_link'] ?? 'https://registrar.artsci.edu/academic-calendar';
