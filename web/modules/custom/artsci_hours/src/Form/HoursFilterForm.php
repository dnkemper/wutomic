...
/**
 * Provides an artsci_hours filter form.
 */
class HoursFilterForm extends FormBase {
  ...
  public function getFormId() {
    static $count;
    $count++;
    return 'artsci_hours_filter_form_' . $count;
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $config = NULL) {
    $form['#attached']['library'][] = 'artsci_hours/filter_form';
    $form['#attributes']['class'][] = 'form-inline clearfix artsci-hours-filter-form';
    ...
    if ((int) $config['display_datepicker'] === 1) {
      $form['#attached']['library'][] = 'artsci_hours/finishedinput';
