...
namespace Drupal\artsci_hours\Plugin\Block;
...
class HoursBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    $build['form'] = $this->formBuilder->getForm('Drupal\artsci_hours\Form\HoursFilterForm', $config);
  ...
  public function blockForm($form, FormStateInterface $form_state) {
    ...
    $config = $this->getConfiguration();
    $group = $this->configFactory->get('artsci_hours.settings')->get('group');
    ...
    if (empty($group)) {
      $form['no-group'] = [
        ...
        '#markup' => $this->t('A resource group must be selected. <a href=":url">Hours configuration</a>.', [
          ':url' => Url::fromRoute('artsci_hours.settings')->toString(),
        ]),
      ];
