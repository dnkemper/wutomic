...
(function (Drupal, once) {
  ...
  window.uiProfiles = window.uiProfiles || { basePath: drupalSettings.artsciProfiles.basePath };

  Drupal.artsciProfiles = {};

  // Updates the canonical link for a person and the metadata descriptions for profiles.
  Drupal.artsciProfiles.updateSEOData = function (settings, url) {
    url = new URL(url);
    ...
    if (path !== settings.artsciProfiles.basePath) {
      ...
      let endpoint = drupalSettings.artsciProfiles.endpoint;

      // Set query parameters, in this case the api key gotten from settings.
      let params = 'api-key=' + drupalSettings.artsciProfiles.api_key;
    ...
    // Else if this is not an individual profile page...
      ...
      let site_name = drupalSettings.artsciProfiles.siteName;
      let directory_title = drupalSettings.artsciProfiles.directoryTitle;
  ...
  Drupal.artsciProfiles.personMetaElement = function (name, directoryTitle) {
  ...
  Drupal.artsciProfiles.directoryMetaElement = function (siteName, directoryTitle) {
    let element = document.createElement('meta');
    element.name = 'description';
   ...
   * Profiles behavior.
   */
  Drupal.behaviors.artsciProfiles = {
    attach(context, settings) {
      once('artsciProfiles', context.querySelector('#uiprof')).forEach(function (profiles) {
        Drupal.artsciProfiles.updateSEOData(settings, document.URL);
