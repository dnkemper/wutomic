...
namespace Drupal\artsci_profiles;
...
class Client {
  ...
  /**
   * The artsci_profiles logger channel.
   *
  ...
  public function __construct(ConfigFactoryInterface $config_factory, LoggerInterface $logger, HttpClient $httpClient) {
    $this->config = $config_factory->get('artsci_profiles.settings');
    $this->logger = $logger;
  ...
  protected function setEndpoint() {
    ...
    if ($this->environment === 'test') {
      $endpoint = 'https://profiles-test.artsci.edu/api';
    }
    elseif ($this->environment === 'prod') {
      $endpoint = 'https://profiles.artsci.edu/api';
    }
