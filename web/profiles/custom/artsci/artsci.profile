...
use Drupal\artsci\Plugin\WebformHandler\EmailOverrideWebformHandler;
...
function artsci_preprocess_html(&$variables) {
  ...
  $meta_web_author = [
    ...
    '#attributes' => [
      'name' => 'web-author',
      'content' => t('Artsci @version (https://artsci.artsci.edu)', [
        '@version' => $version,
      ]),
    ],
  ];
 ...
 */
function artsci_toolbar_alter(&$items) {
 ...
 * Implements hook_preprocess_HOOK().
 */
function artsci_preprocess_breadcrumb(&$variables) {
 ...
 */
function artsci_preprocess_select(&$variables) {
  $admin_context = \Drupal::service('router.admin_context');
  if ($admin_context->isAdminRoute()) {
    if ($variables['element']['#multiple'] === TRUE) {
      // Use chosen for multi-selects.
      $variables['#attached']['library'][] = 'artsci/chosen';
      // Remove none option.
 ...
 */
function artsci_module_implements_alter(&$implementations, $hook) {
  // Unset administerusersbyrole query alter which over-filters the people page.
  // @todo Refactor this to move artsci last and then alter the altered query.
  //   See https://github.com/artsci/artsci/issues/5023
 ...
 */
function artsci_query_administerusersbyrole_edit_access_alter(AlterableInterface $query) {
  ...
  if ($access->isForbidden()) {
    ...
    if ($query instanceof SelectInterface) {
      ...
      // Get a list of artsci with the administrator role.
      ...
      // Exclude those artsci from the result list.
 ...
 */
function artsci_form_menu_edit_form_alter(&$form, FormStateInterface $form_state, $form_id) {

  if ($form['id']['#default_value'] === 'top-links') {
    $theme = \Drupal::config('system.theme')->get('default');
    if (in_array($theme, ['atomic_artsci'])) {
      $limit = theme_get_setting('header.top_links_limit', 'atomic_artsci');
      if ($limit) {
...
/**
 * Custom submit handler for artsci_form_block_form_alter().
 */
function artsci_block_form_submit($form, FormStateInterface $form_state) {
 ...
 */
function artsci_theme_suggestions_block_alter(&$suggestions, $variables) {
 ...
 */
function artsci_preprocess_block(&$variables) {
 ...
 */
function artsci_form_node_confirm_form_alter(&$form, FormStateInterface $form_state) {
  ...
  if ($form_object instanceof EntityFormInterface) {
    ...
    if ($url->isRouted()) {
      ...
      if (isset($params['node']) && $params['node'] === $node->id()) {
        ...
        _artsci_prevent_front_delete_message($node->label());
      }
    }
  }
}
 ...
 * Implements hook_form_FORM_ID_alter().
 */
function artsci_form_node_delete_multiple_confirm_form_alter(&$form) {
  ...
  if (isset($params['node'])) {
    ...
    foreach ($form['entities']['#items'] as $item => $title) {
      ...
      if ($params['node'] === $item[0]) {
        // Disable the 'Delete' button.
        $form['actions']['submit']['#disabled'] = TRUE;
        _artsci_prevent_front_delete_message($title);
        break;
      }
    }
  }
 ...
 * Implements hook_form_FORM_ID_alter().
 */
function artsci_form_views_exposed_form_alter(&$form, FormStateInterface $form_state, $form_id) {
 ...
 * Implements hook_form_FORM_ID_alter().
 */
function artsci_form_views_form_administerusersbyrole_people_page_1_alter(&$form, FormStateInterface $form_state, $form_id) {
  /** @var Drupal\artsci_core\Access\ArtsciCoreAccess $check */
  $check = \Drupal::service('artsci_core.access_checker');
 ...
 * Implements hook_form_FORM_ID_alter().
 */
function artsci_form_config_split_edit_form_alter(&$form, FormStateInterface $form_state, $form_id) {
 ...
 */
function artsci_config_split_prepare_form(EntityInterface $entity, $operation, FormStateInterface $form_state) {
 ...
 */
function _artsci_node_form_defaults(&$form, $form_state) {
  ...
  if (isset($form['field_teaser'])) {
    ...
    if (artsci_get_version() === 'v3' || !str_starts_with($form['#id'], 'node-page')) {
  ...
  if (isset($form['field_featured_image_display'])) {
    ...
    if ($form_object && $node = $form_object->getEntity()) {
      $type = $node->getType() . 's';
      $form['field_featured_image_display']['widget']['#description'] .= t('&nbsp;If "Site-wide default" is selected, this setting can be changed on the <a href="@settings_url">Artsci @types settings</a>.', [
        '@settings_url' => Url::fromRoute("artsci_$type.settings_form")->toString(),
        '@types' => ucfirst($type),
      ]);
    }
  }
 ...
 */
function artsci_form_alter(&$form, FormStateInterface $form_state, $form_id) {
  $form_object = $form_state->getFormObject();
  ...
  switch ($form_id) {
    ...
    case 'webform_ui_element_form':
      ...
      if (isset($form['properties']['captcha'])) {
        ...
        $form['properties']['captcha']['captcha_type']['#description'] = t('See our <a href="@link" target="_blank" rel="noopener">Artsci CAPTCHA documentation</a> for more information.', [
          '@link' => 'https://artsci.artsci.edu/node/646#captcha',
        ]);
      }

      // Custom validation for webform components.
      $form['#validate'][] = '_artsci_webform_validate';
      break;

    // Remove access to headline field in footer contact block.
    case 'block_content_artsci_text_area_edit_form':
      if ($form_object instanceof EntityFormInterface) {
        ...
        if ($uuid === '0c0c1f36-3804-48b0-b384-6284eed8c67e') {
          $form['field_artsci_headline']['#access'] = FALSE;
          if ($access->isForbidden()) {
            $form['field_artsci_text_area']['widget'][0]['#allowed_formats'] = [
              'minimal',
              'plain_text',
            ];
          }
 ...
 *   The form state.
 */
function _artsci_webform_validate(array &$form, FormStateInterface $form_state) {
 ...
 */
function artsci_webform_element_alter(array &$element, FormStateInterface $form_state, array $context) {
  if (isset($element['#webform_key'])) {
    ...
    if (isset($element['#prepopulate'])) {
      ...
      if (in_array($element['#webform_key'], $webform_prepopulate_query_keys)) {
        ...
        $element['#attached']['drupalSettings']['artsci']['webformPrepopulateQueryKeys'] = $webform_prepopulate_query_keys;
        $element['#attached']['library'][] = 'artsci/get_clickid';
      }
    }
  }
}
 ...
 * Implements hook_form_FORM_ID_alter().
 */
function artsci_form_revision_overview_form_alter(&$form, FormStateInterface $form_state, $form_id) {
 ...
 * Implements hook_form_FORM_ID_alter().
 */
function artsci_form_system_site_information_settings_alter(&$form, FormStateInterface $form_state, $form_id) {
  ...
  if ($default_theme === 'atomic_artsci') {
    $form['artsci_footer_block'] = [
      '#type' => 'details',
    ...
    if ($footer_contact_block) {
      ...
      $form['artsci_footer_block']['artsci_footer_contact_info_edit'] = [
        '#type' => 'item',
    ...
    if (array_key_exists($social_media_menu, $menus)) {
      ...
      $form['artsci_footer_menus']['artsci_footer_social_media_menu_help'] = [
        '#type' => 'item',
    ...
    if (array_key_exists($custom_menu, $menus)) {
      ...
      $form['artsci_footer_menus']['artsci_footer_custom_menu_help'] = [
        '#type' => 'item',
    ...
    if (array_key_exists($custom_menu_2, $menus)) {
      ...
      $form['artsci_footer_menus']['artsci_footer_custom_menu_2_help'] = [
        '#type' => 'item',
    ...
    if (array_key_exists($custom_menu_3, $menus)) {
      ...
      $form['artsci_footer_menus']['artsci_footer_custom_menu_3_help'] = [
        '#type' => 'item',
    ...
    // Hide the site slogan field, as it's not used in atomic_artsci theme.
 ...
 *
 * @see artsci_form_node_confirm_form_alter()
 * @see artsci_form_node_delete_multiple_confirm_form_alter()
 */
function _artsci_prevent_front_delete_message($title) {
 ...
 * Implements hook_theme_suggestions_HOOK_alter().
 */
function artsci_preprocess_page(&$variables) {
  $admin_context = \Drupal::service('router.admin_context');
  if (!$admin_context->isAdminRoute()) {
    $node = \Drupal::routeMatch()->getParameter('node');
    $node = ($node ?? \Drupal::routeMatch()->getParameter('node_preview'));
 ...
 * Implements hook_preprocess_HOOK().
 */
function artsci_preprocess_node(&$variables) {
  $admin_context = \Drupal::service('router.admin_context');
  if (!$admin_context->isAdminRoute()) {
    $node = $variables['node'];
 ...
 */
function artsci_form_menu_link_content_form_alter(array &$form, FormStateInterface $form_state, $form_id) {
  $form_object = $form_state->getFormObject();
      ...
      if ($link instanceof FieldItemList) {
        ...
        if ($menu === 'social') {
          ...
          $form['fa_icon'] = [
            ...
            '#attached' => [
              'library' => [
                'artsci/vanilla-icon-picker',
              ],
            ],
          ];

          $form['actions']['submit']['#submit'][] = 'artsci_form_menu_link_content_form_submit';
        }
      }
    }
  }
...
/**
 * Custom validation function for artsci_form_menu_link_content_form_alter.
 *
 * @throws \Drupal\Core\TypedData\Exception\MissingDataException
 */
function artsci_form_menu_link_content_form_submit(array &$form, FormStateInterface $form_state) {
 ...
 */
function artsci_link_alter(&$variables) {
 ...
 * Implements hook_preprocess_HOOK().
 */
function artsci_preprocess_field(&$variables) {
 ...
 */
function artsci_page_attachments(array &$attachments) {
  // Attach css file on admin pages.
  $admin_context = \Drupal::service('router.admin_context');
  $admin_theme = \Drupal::config('system.theme')->get('admin');

  if ($admin_context->isAdminRoute() && $admin_theme === 'claro') {
    $attachments['#attached']['library'][] = 'artsci/admin-overrides';
  }
}
...
function artsci_toolbar() {
  $version = artsci_get_version();
  $url = Url::fromUri('//artsci.artsci.edu/node/36');
  ...
  $items['support'] = [
    ...
    'tab' => [
      '#type' => 'link',
      '#url' => $url,
      '#title' => t('Artsci @version Help', [
        '@version' => $version,
      ]),
      '#options' => [
        'attributes' => [
          'title' => t('Opens help documentation in a new window.'),
          'id' => 'toolbar-item-artsci-help',
          'class' => [
            'toolbar-item',
          ],
          'role' => 'button',
...
/**
 * Determine the version of Artsci based on what config is active.
 *
 * @todo Return additional information like if any other splits are active that might impact functionality.
 * See https://github.com/artsci/artsci/issues/5021
 */
function artsci_get_version() {
  $version = 'v3';

  $is_v2 = \Drupal::config('config_split.config_split.artsci_v2')->get('status');
 ...
 */
function artsci_webform_handler_info_alter(array &$handlers) {
